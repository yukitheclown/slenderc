...
static void CreateShader(struct Shader *shader, const char *vSource, const char *fSource){
    ...
    shader->useFlashlightLoc = glGetUniformLocation(shader->program, "flashlightOn");
...
static const char *textureless2DVSource = "#version 120\n"
"attribute vec2 pos;\n"
...
static const char *textured2DFSource = "#version 120\n"
"uniform vec4 uniColor = vec4(1,1,1,1);\n"
...
static const char *textured2DVSource = "#version 120\n"
"attribute vec2 pos;\n"
"attribute vec2 coord;\n"
"attribute vec4 color;\n"
"uniform mat4 projection;\n"
...
"uniform vec4 uniColor = vec4(1,1,1,1);\n"
"uniform vec2 reflectionVector;\n"
"uniform float blurAmount = 0;\n"
"uniform sampler2D tex;\n"
"varying vec2 TexCoord;\n"
"void main(){\n"
    "vec4 t = texture2D(tex, TexCoord);\n"
    // "if(t.r < 0.1) discard;\n"
    "gl_FragColor = vec4(uniColor.x, uniColor.y, uniColor.z, t.r) * colorFromVertShader;\n"
    
    // "gl_FragData[0] = vec4(uniColor.x, uniColor.y, uniColor.z, t.r) * colorFromVertShader;\n"
    // "gl_FragData[1] = vec4(Position_CamSpace,1);\n"
    // "gl_FragData[2] = vec4(reflectionVector, 0, 1);\n"
    // "gl_FragData[3] = vec4(blurAmount, 0, 0, 1);\n"
"}\n";
...
"varying vec2 TexCoord;\n"
"void main(){\n"
    "Position_CamSpace = (projection * view * model * vec4(pos, 1)).xyz;\n"
    "gl_Position = vec4(Position_CamSpace,1);\n"
    "colorFromVertShader = color;\n"
    "TexCoord = coord;"
"}\n";

static const char *text3DFSource = "#version 120\n"
"varying vec4 colorFromVertShader;\n"
...
static const char *text3DVSource = "#version 120\n"
"attribute vec2 pos;\n"
"attribute vec2 coord;\n"
...
static const char *hardShadowsFSource = "#version 120\n"
"varying vec2 TexCoord;\n"
"varying vec4 ShadowCoord;\n"
"varying vec3 Normal_Worldspace;\n"
"varying vec3 Normal_CamSpace;\n"
...
"uniform sampler2D tex;\n"
"uniform sampler2DShadow shadowMap;\n"
"uniform vec4 uniColor = vec4(0.05,0.05,0.05,1);\n"
...
"void main(){\n"
    ...
    "vec3 camPosToFragVec = vec3(invView * vec4(0,0,0,1) - Position);\n"
    "vec3 viewVec = vec3(invView * vec4(0,0,1,0));\n"
    "float fogginess = length(camPosToFragVec)/50;\n"
    ...
    "if(useShadows){\n"
        ...
        "visibility.rgb *= (1-fogginess);\n"
        "gl_FragData[0] = visibility * (uniColor * texColor);\n"

    "} else {\n"
        "gl_FragData[0] = (uniColor * texColor);\n"
            
        "if(flashlightOn){\n"
            "float theta = acos(dot(normalize(viewVec), normalize(camPosToFragVec)));\n"
            
            "if(theta < 0.65){\n"

                "gl_FragData[0].rgb += texColor.rgb * ((1-(theta/0.65)) * (vec3(0.4,0.3,0.2)));\n"
            "}\n"
        "}\n"

        "gl_FragData[0].rgb *= (1-fogginess);\n"
    "}\n"
...
"vec4 GetReflectionColor(){\n"
    ...
    "vec3 cameraSpacePos = texture2D(positionMap, TexCoord.xy).xyz;"
    "vec3 cameraSpaceNorm = texture2D(normalMap, TexCoord.xy).xyz;"
    ...
    "for(k = 0; k < 200; k++){\n"
        ...
        "float depth = texture2D(depthMap, rayPos.xy).r;\n"
    ...
    "if(k < 200) return texture2D(colorMap, rayPos.xy);\n" 

    "return texture2D(colorMap, TexCoord.xy);\n"
"}\n"

"void main(){\n"

    "vec4 color = texture2D(colorMap, TexCoord.xy);\n"
    "float reflectionAmount = texture2D(fragAttribMap, TexCoord.xy).g;\n"
    ...
    "if(rgbSplitAmount != 0){\n"
        "color.r /= 2;\n"
        "color.b /= 2;\n"
        "color.r += texture2D(colorMap, TexCoord + vec2(rgbSplitAmount, 0)).r/2;\n"
        "color.b += texture2D(colorMap, TexCoord + vec2(-rgbSplitAmount, 0)).b/2;\n"
    "}\n"

    // "float glowAmount = texture2D(fragAttribMap, TexCoord).b/5;\n"
    // "glowAmount += texture2D(fragAttribMap, vec2(TexCoord.x+(0.01), TexCoord.y)).b/5;\n"
    // "glowAmount += texture2D(fragAttribMap, vec2(TexCoord.x-(0.01), TexCoord.y)).b/5;\n"
    // "glowAmount += texture2D(fragAttribMap, vec2(TexCoord.x, TexCoord.y+(0.01))).b/5;\n"
    // "glowAmount += texture2D(fragAttribMap, vec2(TexCoord.x, TexCoord.y-(0.01))).b/5;\n"
    // "color += glowAmount;\n"
...
static const char *softShadowsFSource = "#version 120\n"
...
"void FindBlocker(varying float avgBlockerDepth, inout float numBlockers, vec2 uv, float zReceiver){\n"
...
"void main(){\n"
    ...
    "if(useShadows){\n"
        ...
        "}\n"
    "}\n"
    "gl_FragData[0] = visibility * (vec4(uniColor.xyz, 1) + vec4(uniColor.a*texture2D(tex, TexCoord.xy).xyz,1));\n"
...
static const char *skeletalVSource = "#version 120\n"
"attribute vec3 pos;\n"
"attribute vec2 coord;\n"
"attribute vec3 norm;\n"
...
"void main(){\n"
    ...
    "Normal_Worldspace = (invTrans * norm).xyz;\n"
    "Normal_CamSpace = vec3(normalize(view * vec4(Normal_Worldspace,0)));\n"
    "Position = model * matTranform * vec4(pos,1);\n"
    "Position_CamSpace = (view * Position).xyz;\n"
    "TexCoord = coord;\n"
    "gl_Position = projection * vec4(Position_CamSpace, 1);\n"
    "ShadowCoord = bias * depthMvp * Position;\n"
...
static const char *particleVSource = "#version 120\n"
"attribute vec2 pos;\n"
"attribute vec2 billboardSize;\n"
"attribute vec3 billboardCenter;\n"
"attribute vec4 color;\n"
...
void Shaders_Init(){
    ...
    int k;
    for(k = 0; k < TEXTURED_2D_SHADER+1; k++)
...
void Shaders_SetUseFlashlight(int use){
    if(activeProgram == (GLuint)-1 || shaders[activeProgram].useFlashlightLoc == (GLuint)-1) return;
    glUniform1i(shaders[activeProgram].useFlashlightLoc, use);
}
